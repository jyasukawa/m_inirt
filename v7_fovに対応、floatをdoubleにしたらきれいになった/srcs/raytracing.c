                      ...
                      intersection_point_t* out_intp)
{
  if ( shape->type == ST_SPHERE )
    ...
    else if ( shape->type == ST_CYLINDER )
    {
      ...
      double aa = squared_norm(&dm);//dot(&ray->direction, &ray->direction);と結果は同じ
      double bb = 2 * dot(&dm, &mm);
      double cc = squared_norm(&mm) - cylin->radius * cylin->radius;
      
      double dd = SQR(bb) - 4 * aa * cc;
      ...
      if (tt > 0)
      {
        if (out_intp)
        {
          ...
          double hdist = sqrt(SQR(cylin->position.y - out_intp->position.y));
